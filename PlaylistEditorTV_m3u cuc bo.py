...
def process_m3u(content, url=""):
    ...
    for line in lines:
        ...
        if line.startswith("#EXTINF"):
            ...
            # Kiểm tra xem channel_info đã được khởi tạo chưa
...
# Hàm tải danh sách IPTV từ URL hoặc file cục bộ
def load_iptv_list(url):
    try:
        ...
        messagebox.showerror("Lỗi", f"Không tìm thấy file: {url}")
        return []
    except Exception as e:
        messagebox.showerror("Lỗi", f"Lỗi khi đọc file: {e}")
        return []
...
def display_channels(filtered_list=None):
    ...
    for channel in data:
        # Sử dụng phương thức get() để truy cập các khóa một cách an toàn
        stt = channel.get("STT", "N/A")
        ...
        item_id = tree.insert("", tk.END, values=(stt, tvg_id, name, tvg_Name, tvg_logo, group_title, url))
...
def check_url_status(url):
    # Kiểm tra nếu URL là một file cục bộ
    if os.path.exists(url):
        messagebox.showinfo("Thông báo", "Không thể kiểm tra trạng thái của file cục bộ.")
        return
...
def check_url_status_and_color(item_id, url):
    # Không kiểm tra các file cục bộ
    if os.path.exists(url):
        return
...
# Định dạng cột
column_widths = [40, 100, 150, 100, 100, 100, 200]
for i, col in enumerate(columns):
